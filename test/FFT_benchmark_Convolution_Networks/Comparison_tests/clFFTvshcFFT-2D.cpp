...
void hcfft_2d_r2c(size_t N1, size_t N2) {
  ...
  cout << "HCFFT Kernel execution time R2C Transform for size " << N1 << "x"
       << N2 << " in <ms>:" << time_in_ms << endl;
...
void clfft_2d_r2c(size_t N1, size_t N2) {
  ...
  bufX =
      clCreateBuffer(ctx, CL_MEM_READ_WRITE, realSize * sizeof(*X), NULL, &err);
  assert(err == CL_SUCCESS);
  bufY = clCreateBuffer(ctx, CL_MEM_READ_WRITE, complexSize * sizeof(*Y), NULL,
                        &err);
  assert(err == CL_SUCCESS);
  err = clEnqueueWriteBuffer(queue, bufX, CL_TRUE, 0, realSize * sizeof(*X), X,
                             0, NULL, NULL);
  assert(err == CL_SUCCESS);
  err = clEnqueueWriteBuffer(queue, bufY, CL_TRUE, 0, complexSize * sizeof(*Y),
                             Y, 0, NULL, NULL);
  assert(err == CL_SUCCESS);
  /*------------------------------------------------------R2C--------------------------------------------------------------------*/
  ...
  double Avg_time = average(elapsed_pfe);
  double time_in_ms = Avg_time * 1e3;
  cout << "CLFFT Kernel execution time R2C Transform for size " << N1 << "x"
       << N2 << " in <ms>:" << time_in_ms << endl;
  /* Fetch results of calculations. */
  err = clEnqueueReadBuffer(queue, bufY, CL_TRUE, 0, complexSize * sizeof(*Y),
                            Y, 0, NULL, NULL);
  assert(err == CL_SUCCESS);
...
void hcfft_2d_c2r(size_t N1, size_t N2) {
  ...
  status =
      plan.hcfftSetLayout(planhandle, HCFFT_HERMITIAN_INTERLEAVED, HCFFT_REAL);
  ...
  double Avg_time = average(elapsed_pfe);
  double time_in_ms = Avg_time * 1e3;
  cout << "HCFFT Kernel execution time C2R Transform for size " << N1 << "x"
       << N2 << " in <ms>:" << time_in_ms << endl;
  // Copy Device output  contents back to host
...
void clfft_2d_c2r(size_t N1, size_t N2) {
  ...
  double Avg_time = average(elapsed_pfe);
  double time_in_ms = Avg_time * 1e3;
  cout << "CLFFT Kernel execution time C2R Transform for size " << N1 << "x"
       << N2 << " in <ms>:" << time_in_ms << endl;
  /* Fetch results of calculations. */
  err = clEnqueueReadBuffer(queue, bufX, CL_TRUE, 0, realSize * sizeof(*X), X,
                            0, NULL, NULL);
  assert(err == CL_SUCCESS);
  /* Release OpenCL memory objects. */
  clReleaseMemObject(bufX);
...
void hcfft_2d_c2c(size_t N1, size_t N2) {
  ...
  /*---------------------C2C--------------------------------------*/
  status = plan.hcfftSetLayout(planhandle, HCFFT_COMPLEX_INTERLEAVED,
  ...
  double Avg_time = average(elapsed_pfe);
  double time_in_ms = Avg_time * 1e3;
  cout << "HCFFT Kernel execution time C2C Transform for size " << N1 << "x"
       << N2 << " in <ms>:" << time_in_ms << endl;
  // Copy Device output  contents back to host
...
void clfft_2d_c2c(size_t N1, size_t N2) {
  ...
  assert(err == CL_SUCCESS);
  err = clEnqueueWriteBuffer(queue, bufX, CL_TRUE, 0, size * sizeof(*X), X, 0,
                             NULL, NULL);
  assert(err == CL_SUCCESS);
  err = clEnqueueWriteBuffer(queue, bufY, CL_TRUE, 0, size * sizeof(*Y), Y, 0,
                             NULL, NULL);
  assert(err == CL_SUCCESS);
  /*------------------------------------------------------C2C--------------------------------------------------------------------*/
  /* Create a default plan for a complex FFT. */
  ...
  double Avg_time = average(elapsed_pfe);
  double time_in_ms = Avg_time * 1e3;
  cout << "CLFFT Kernel execution time C2C Transform for size " << N1 << "x"
       << N2 << " in <ms>:" << time_in_ms << endl;
  /* Fetch results of calculations. */
  err = clEnqueueReadBuffer(queue, bufY, CL_TRUE, 0, size * sizeof(*Y), Y, 0,
                            NULL, NULL);
  assert(err == CL_SUCCESS);
  /* Release OpenCL memory objects. */
  clReleaseMemObject(bufX);
