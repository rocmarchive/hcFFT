...
TEST(hcfft_2D_transform_test, func_correct_2D_transform_Z2D_RTT) {
  ...
  hcDoubleComplex* outputD2Z = (hcDoubleComplex*)calloc(Csize, sizeof(hcDoubleComplex));
  ...
  hcDoubleComplex* devOpD2Z = hc::am_alloc(Csize * sizeof(hcDoubleComplex), accs[1], 0);
  accl_view.copy(outputD2Z, devOpD2Z, sizeof(hcDoubleComplex) * Csize);
  status = hcfftExecD2Z(*plan, devIpD2Z, devOpD2Z);
  EXPECT_EQ(status, HCFFT_SUCCESS);
  accl_view.copy(devOpD2Z, outputD2Z, sizeof(hcDoubleComplex) * Csize);
  ...
  EXPECT_EQ(status, HCFFT_SUCCESS);
  hcDoubleComplex* inputZ2D = (hcDoubleComplex*)calloc(Csize, sizeof(hcDoubleComplex));
  ...
  hcDoubleComplex* devIpZ2D = hc::am_alloc(Csize * sizeof(hcDoubleComplex), accs[1], 0);
  accl_view.copy(inputZ2D, devIpZ2D, sizeof(hcDoubleComplex) * Csize);
