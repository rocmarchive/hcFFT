...
if [ ! -d "$working_dir1" ]; then
  echo "Please run script[test.sh] from hcfft/test/unit-hip/"
  exit
fi
cd $working_dir1
...
#Gtest functions
unittest="$working_dir1/hipfft_Create_destroy_Plan"
...
if [ ! -s "$Log_file" ]; then
  ...
  then
    echo "${red}hipfft_Create_destroy_Plan               ----- [ FAILED ]${reset}"
  elif grep -q PASSED "$Log_file";
  then
    echo "${green}hipfft_Create_destroy_Plan             ----- [ PASSED ]${reset}"
    rm -f Log_file
  fi
fi

test_1d_transforms_file=($current_work_dir/hipfft_transforms/hipfft_1D_transform*.cpp)
...
## now loop through the above array
  ...
  do
    ...
    working_dir1="$current_work_dir/../../build/test/unit-hip/hipfft_transforms/bin"
    cd $working_dir1
    if [ ! -d "errlog" ]; then
      mkdir "errlog"
    fi
...
test_2d_transforms_file=($current_work_dir/hipfft_transforms/hipfft_2D_transform*.cpp)
...
test_3d_transforms_file=($current_work_dir/hipfft_transforms/hipfft_3D_transform*.cpp)
