...
# Move to gtest bin
working_dir1="$current_work_dir/../../build/test/unit-api/hcfft_Create_Destroy_Plan/bin"
if [ ! -d "$working_dir1" ]; then
  echo "Please run script[test.sh] from hcfft/test/unit-api/"
  exit
fi
cd $working_dir1
...
## now loop through the above array
  ...
  do
    ...
    working_dir1="$current_work_dir/../../build/test/unit-api/hcfft_transforms/bin"
    cd $working_dir1
    if [ ! -d "errlog" ]; then
      mkdir "errlog"
    fi
