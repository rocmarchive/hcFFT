...
//std::cout << " transkernel " << transKernel.str().c_str() << std::endl;
...
hcfftStatus FFTPlan::GenerateKernelPvt<Transpose_GCN>(const hcfftPlanHandle plHandle, FFTRepo& fftRepo, size_t count, bool exist) const {
  ...
  if(!exist)
  {
//std::cout << " generatekernel gcn "<< plHandle << std::endl;
  	size_t loopCount = 0;
	  tile blockSize = {0, 0};
  	CalculateBlockSize(fftParams.fft_precision, loopCount, blockSize);
...
//std::cout << " out of gentranspose " << plHandle << std::endl;
//std::cout << " programcode " << programCode << std::endl;
